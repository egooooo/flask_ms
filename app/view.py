...
from app.models import DeviceTest
from app import db
...
class DeviceTestStatView(Resource):
    def get(self):
        operator = request.args.get('operator')
        print(f"operator: {operator}")

        if operator is not None:
            device_test = DeviceTest.query.filter_by(operator=operator).all()

            return make_response(
                jsonify({"result": get_stat(device_test)}), 200
            )

        device_test = DeviceTest.query.all()
        return make_response(jsonify({"result": get_stat(device_test)}), 200)


class DeviceTestView(Resource):
    def post(self):
        data = request.json
        ...
        return make_response(
            jsonify({"result": {"DeviceTest ID": device_test.id}}), 200
        )


class DeviceTestDeleteView(Resource):
    def delete(self, device_test_id):
        device_test = DeviceTest.query.filter_by(id=device_test_id).first()
